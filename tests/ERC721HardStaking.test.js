...
describe('ERC721HardStaking Tests', function () {
  before(async function() {
    ...
    const staking = await deploy('ERC721HardStaking', nft.address, token.address, ethers.utils.parseEther('0.0001'))
    
    await bindContract('withNFT', 'ERC721Mock', nft, signers)
    await bindContract('withToken', 'ERC20Mock', token, signers)
    await bindContract('withStaking', 'ERC721HardStaking', staking, signers)
  ...
  it('Should stake NFT', async function() {
    ...
    const token = await admin.withStaking.stakers(staker.address, 0)
  ...
  it('Should be releasable', async function() {
    ...
    expect(await admin.withStaking.releaseable(1)).to.be.above(
      ethers.utils.parseEther(String(0.0001 * 3600 * 24 * 30))
    )
  })
