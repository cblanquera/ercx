...
interface IERC20Mintable is IERC20 {
  ...
  function approvable(uint256 tokenId, bool yes) external;
}

interface IERC721ApprovableTotalSupply is IERC721 {
  function approvable(uint256 tokenId, bool yes) external;
 ...
 */
contract ERC721SoftStakingLocked is Context, ReentrancyGuard {
  //used in unstake()
  ...
  IERC721ApprovableTotalSupply public immutable NFT_ADDRESS;
  ...
  constructor(
    IERC721ApprovableTotalSupply nftAddress, 
  ...
  function stake(uint256[] memory tokenIds) public virtual {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      //prevent the staker from listing this out
      NFT_ADDRESS.approvable(tokenId, false);
  ...
  function unstake(
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      //if not owner
      if (NFT_ADDRESS.ownerOf(tokenId) != staker) 
        revert InvalidCall();
      //add releasable
      toRelease += releaseable(tokenId);
      //allow the staker to list now
      NFT_ADDRESS.approvable(tokenId, true);
